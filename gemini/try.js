...
const GEMINI_KEY = process.env.API_KEY || 'AIzaSyCD36kuVVNNteZ-4LMUl0kAJhFgShcZ8Lo'; // GEMINI 的 Key
const GEMINI_MODEL = process.env.MODEL || 'gemini-2.0-flash-exp'; // 使用的模型
const GEMINI_MAX_TOKEN = process.env.MAX_TOKEN || 1024; // 最大 token 的值

// GoogleGenerativeAI required config
// const configuration = new GoogleGenerativeAI(GEMINI_KEY);

// const model = configuration.getGenerativeModel({ model: GEMINI_MODEL });
// 替换 Gemini 客户端配置
class GeminiApi {
    constructor(config) {
...
async function reply(messageId, content) {
    try {
        ...
        console.log('send message to feishu error e ---->', e)
        logger('send message to feishu error', e, messageId, content);
    }
}

// 根据sessionId构造用户会话
async function buildConversation(sessionId) {
    let prompt = [];

    // 从 MsgTable 表中取出历史记录构造 question
    const historyMsgs = await MsgTable.where({ sessionId }).find();
    for (const conversation of historyMsgs) {
        prompt.push({ role: 'user', parts: conversation.question });
        prompt.push({ role: 'model', parts: conversation.answer });
    }
...
// 如果历史会话记录大于GEMINI_MAX_TOKEN，则从第一条开始抛弃超过限制的对话
async function discardConversation(sessionId) {
    ...
    for (const c of countList) {
        if (c.totalSize > GEMINI_MAX_TOKEN) {
...
// 通过 GEMINI API 获取回复
async function getGeminiReply(model, prompt, question) {
    try {
        // 构造消息格式
        const messages = [
            ...prompt,  // 历史消息
            { role: "user", content: question }  // 新问题
        ];
        ...
        logger('chat error --->', e.response?.data || e);
...
async function doctor() {
    ...
    if (GEMINI_KEY === '') {
        return {
            code: 1,
            message: {
                zh_CN: '你没有配置 Gemini 的 Key，请检查 & 部署后重试',
                en_US: 'Here is no Gemini Key, please check & re-Deploy & call again',
            },
        };
    }
...
async function handleReply(userInput, sessionId, messageId, eventId) {
    ...
    // 通过历史记录构造prompt
    const prompt = await buildConversation(sessionId);
    const geminiResponse = await getGeminiReply(model, prompt, question);
...
module.exports = async function (params, context) {
    ...
    // 自检查逻辑
    if (
        !params.hasOwnProperty('header') ||
        (context.trigger === 'DEBUG' && !params.debug)
    ) {
        logger('enter doctor');
        return await doctor();
    }
